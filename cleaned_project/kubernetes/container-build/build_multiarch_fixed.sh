...
sed -i '' 's|felipedbene/ai-blogging-butler:latest|fdebene/ai-butler:latest|g' ../cronjob-actual.yaml
...
sed -i '' "s/REDACTED_PASSWORD/$WP_PASSWORD/g" ../secrets/secrets-actual.yaml
envsubst < ../secrets/secrets-actual.yaml > ../secrets/secrets-ready.yaml

# Apply secrets to Kubernetes
kubectl apply -f ../secrets/secrets-ready.yaml

# Apply CronJob to Kubernetes
echo "⏰ Deploying CronJob to Kubernetes..."
kubectl apply -f ../cronjob-actual.yaml

# Clean up sensitive files
rm ../secrets/secrets-ready.yaml
